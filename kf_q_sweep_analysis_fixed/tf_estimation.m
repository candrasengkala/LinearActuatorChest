...
%% --- Load data ---
csv_path = "fixed_sigma_160_extend.csv";  % <-- change if your filename differs
try
...
%% --- Convert G_pos_direct to a PHYSICAL-STATE state-space model ---
% G_pos_direct is an idproc "P1I" model: G(s) = Kp / (s*(1 + Tp1*s))
% Calling ss(G_pos_direct) directly would give MATLAB's own internal
% realization, which does NOT correspond to [position; velocity] states.
% For Simulink initial-condition seeding (matching the Kalman filter's
% state convention), we build the realization by hand instead:
%
%   x1 = position, x2 = velocity
%   x1_dot = x2
%   x2_dot = (Kp*u - x2) / Tp1
%
% This is structurally identical to the earlier hand-derivation from
% 0.0745/((1e-6)s^2 + s), just parameterized via the identified Kp/Tp1.
Kp  = G_pos_direct.Kp;
Tp1 = G_pos_direct.Tp1;

fprintf("=== Identified P1I parameters ===\n");
fprintf("Kp  = %.6g\n", Kp);
fprintf("Tp1 = %.6g s\n\n", Tp1);
...
B_pos = [0;
         Kp/Tp1];
C_pos = [1 0];   % output = position
D_pos = 0;

sys_pos_direct_ss = ss(A_pos, B_pos, C_pos, D_pos);
sys_pos_direct_ss.StateName  = {'position'; 'velocity'};
sys_pos_direct_ss.InputName  = "PWM";
sys_pos_direct_ss.OutputName = "Position";

fprintf("=== State-space realization of G_pos_direct (physical states) ===\n");
sys_pos_direct_ss

% Sanity check: step response of the physical-state ss model should match
% the original idproc G_pos_direct step response (both zero-IC by default).
figure("Name", "G_pos_direct: idproc vs physical-state ss (sanity check)");
[y_proc, t_proc] = step(pwm_step * G_pos_direct, t_fit(end));
[y_ss, t_ss]     = step(pwm_step * sys_pos_direct_ss, t_fit(end));
plot(t_proc, y_proc, "b-", "LineWidth", 1.5, "DisplayName", "G\_pos\_direct (idproc)");
hold on;
plot(t_ss, y_ss, "r--", "LineWidth", 1.5, "DisplayName", "sys\_pos\_direct\_ss (physical states)");
legend("show", "Location", "best");
title("Sanity check: idproc fit vs hand-built physical-state SS realization");
xlabel("Time (s)"); ylabel("Position");
grid on;

fprintf("\nUse A_pos, B_pos, C_pos, D_pos in the Simulink State-Space block.\n");
fprintf("Set Initial conditions = [x0_position; x0_velocity] to match your\n");
fprintf("Kalman filter's state convention, e.g. [position_reading(1); 0].\n\n");
...
fprintf("=== Derived PWM -> Position TF (from 1st-order velocity model, G_vel_1/s) ===\n");
G_pos_from_vel1

fprintf("=== Derived PWM -> Position TF (from 2nd-order velocity model, G_vel_2/s) ===\n");
G_pos_from_vel2

fprintf("=== Direct-fit PWM -> Position TF (procest P1I) ===\n");
G_pos_direct
