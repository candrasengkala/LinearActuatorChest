...
void loop() {
  ...
  timer.deltaTime();
  // --- Core Operations ---
  lin_act.allExtend(&pwm_value); // Automatically retracts the actuator after each loop iteration
  ...
  if (micros() - lastPrint >= 25000) { // Every 25ms
    ...
    filter.stepResponse(&pwm_value, &delta_time); // This will now successfully print your changing velocity variable!
  }
}